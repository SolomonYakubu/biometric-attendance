...
export default function Report() {
  ...
  const getReport = (event) => {
    ...
    if (event.data.profile) {
      ...
      const imgPath =
        ...
        electron.path.resolve("", event.data.profile.dp);

      setImg(imgPath);
      return;
    }
    alert.show("Staff Not Registered", { type: "error" });
  };
  const onSubmit = (e) => {
    e.preventDefault();
    electron.report(e.target.id.value);
  ...
  return (
    <div className="container pt-6 pb-14">
      ...
      <div className="sub-container flex flex-col items-center justify-center p-5 w-4/5">
        <form
          onSubmit={onSubmit}
        ...
        >
          <input
            type={"text"}
            placeholder="Input Staff ID to generate report"
        ...
        <div className="flex justify-between bg-[#20232a] m-3  w-full gap-2 flex-wrap items-start p-3">
          <div className="h-full w-48 mt-3 p-2">
            {
              <>
                {(img && (
                  ...
                  <div className="w-full h-48 bg-[#282c34] text-white text-center  flex  p-2">
                    Profile Image
                  </div>
                )}{" "}
              </>
          ...
          <div className=" p-3 text-white  flex flex-col justify-around items-start flex-1">
            ...
            <div className="  w-100  flex items-center mb-2">
              Staff ID: {(data && data?.profile?.staffID) || "xxxxxxxx"}
            </div>
            <div className="  w-100  flex items-center mb-2">
              Department: {(data && data?.profile?.department) || "xxxxxxxx"}
            </div>
            <div className="  w-100  flex items-center mb-2">
              Email: {(data && data?.profile?.email) || "xxxxxxxx"}
            </div>
            <div className="  w-100  flex items-center mb-2">
